...
4. **Limitation:** serverless functions don't share memory across requests, so the Add to Cart / checkout flow (`frontend/cart.js`, needs multi-step order state) doesn't work through this deployment — only the chat, menu, and hours/location work. `frontend/staff.html` and `/api/order/*` are not available here at all (they only exist on `backend/server.js`).
...
🚧 Ordering backend (with real AI replies via Anthropic's API), a real cafe homepage with a working Add to Cart/checkout flow and a chat widget, a standalone mock chat page, a staff dashboard, and a stateless Vercel serverless deployment option are all built. Real payment processing is not implemented yet.
